...
dynamodb_table = dynamodb.Table('charge_info')
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if http_method == 'GET' and path == status_check_path:
            ...
            charge_id = event['queryStringParameters']['chargeid']
            ...
            response = modify_charge(body['chargeId'], body['updateKey'], body['updateValue'])
        elif http_method == 'DELETE' and path == charge_path:
            body = json.loads(event['body'])
            response = delete_charge(body['chargeId'])
        else:
...
def get_charge(charge_id):
    try:
        response = dynamodb_table.get_item(Key={'chargeid': charge_id})
